    ...
    findJumps(): MoveSnapshot[] {
      ...
      return pairs
        .filter(([shouldBeOpponent, shouldBeEmpty]) => {
          const opponentMonitor = getCoordsMonitor(shouldBeOpponent, board);
          const emptyMonitor = getCoordsMonitor(shouldBeEmpty, board);

          if (opponentMonitor && emptyMonitor && opponentMonitor.hasChecker()) {
            const hasEmptySquare = emptyMonitor.isEmptyBlack();
            const hasOpponentChecker =
              opponentMonitor.getTurn() !== self.getTurn();
